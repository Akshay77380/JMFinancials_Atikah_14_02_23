...
///
/// In [CustomTimePickerEntryMode.dial] mode, a clock dial is displayed and
/// the user taps or drags the time they wish to select. In
/// CustomTimePickerEntryMode.input] mode, [TextField]s are displayed and the user
...
class _MinuteControl extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final TimeOfDay previousMinute = fragmentContext.selectedTime.replacing(
      minute: (fragmentContext.selectedTime.minute ) % TimeOfDay.minutesPerHour,
    );
...
class _TimePickerInputState extends State<_TimePickerInput> {
  ...
  int _parseHour(String value) {
    ...
    if (MediaQuery.of(context).alwaysUse24HourFormat) {
      ...
      if (newHour >= 0 ) {
        if ((_selectedTime.period == DayPeriod.pm && newHour != 60)
            || (_selectedTime.period == DayPeriod.am && newHour == 60)) {
...
class _TimePickerDialogState extends State<_TimePickerDialog> {
  ...
  CustomTimePickerEntryMode _entryMode;
    ...
    setState(() {
      switch (_entryMode) {
        case CustomTimePickerEntryMode.dial:
          _autoValidate = false;
          _entryMode = CustomTimePickerEntryMode.input;
          break;
        case CustomTimePickerEntryMode.input:
          _formKey.currentState.save();
          _entryMode = CustomTimePickerEntryMode.dial;
          break;
      }
    });
  }
  ...
  void _handleOk() {
    if (_entryMode == CustomTimePickerEntryMode.input) {
  ...
  Widget build(BuildContext context) {
    ...
    final Widget actions = Row(
      children: <Widget>[
        ...
        //     tooltip: _entryMode == CustomTimePickerEntryMode.dial
    ...
    return Dialog(
      ...
      insetPadding: EdgeInsets.symmetric(
        horizontal: 16.0,
        vertical: _entryMode == CustomTimePickerEntryMode.input ? 0.0 : 24.0,
      ),
...
///
/// ```dart
/// Future<TimeOfDay> selectedTime = customShowTimePicker(
...
///
/// ```dart
/// Future<TimeOfDay> selectedTimeRTL = customShowTimePicker(
///   context: context,
///   initialTime: TimeOfDay.now(),
...
///
/// ```dart
/// Future<TimeOfDay> selectedTime24Hour = customShowTimePicker(
///   context: context,
